...
export const onlyIdentityGroupManagerFromPath = (): OnlyIdentityGroupManager<any, any> => ({
  before: async handler => {
    ...
    const identityGroupId = event.pathParameters?.identityGroup;
    if (!identityGroupId) throw new BadRequest("`identityGroup` is not found in path");
...
export const onlyIdentityGroupManagerFromBody = (): OnlyIdentityGroupManager<any, any> => ({
  before: async handler => {
    const { event } = handler;
